    ...
    <PageLayout>
      <section className="hero-grad border-b border-border">
        <div className="container-x py-16">
          <h1 className="font-display text-5xl md:text-6xl max-w-3xl">Browse by community — across every category, in any language.</h1>
          <p className="mt-4 text-lg text-muted-foreground max-w-2xl">A Polish-speaking plumber, a Polish delicatessen caterer, a Polish-speaking hairdresser — three different categories, one community, surfaced together.</p>
        </div>
      </section>
      <div className="container-x py-12 grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {communities.map(c => (
          <Link key={c.slug} to="/community/$slug" params={{slug:c.slug}} className="card-soft p-6 flex flex-col gap-2 hover:-translate-y-0.5 transition-transform">
