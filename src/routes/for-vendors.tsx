      ...
      <section className="border-b border-border" style={{background:"linear-gradient(135deg, var(--amber-soft), var(--teal-soft) 80%)"}}>
        <div className="container-x py-20 grid lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <h1 className="mt-3 font-display text-5xl md:text-7xl leading-[0.95]">Get found by every customer near you.</h1>
            <p className="mt-5 text-lg text-muted-foreground max-w-xl">List your service in 10 minutes. No monthly fees, no contracts. Pay a flat 18% only when you get booked — Ireland's most vendor-friendly commission.</p>
            <div className="mt-7 flex flex-wrap gap-3">
              <Link to="/vendor-dashboard" className="px-6 py-3.5 rounded-full bg-ink text-cream font-semibold text-sm inline-flex items-center gap-2">Start listing <ArrowRight className="size-4"/></Link>
              <Link to="/vendor-dashboard" className="px-6 py-3.5 rounded-full border border-border bg-card font-semibold text-sm">See vendor dashboard</Link>
            </div>
          </div>
          <div className="card-soft p-7">
            ...
            <div className="mt-5 space-y-4">
              {[
                ...
                <div key={r.c}>
                  <div className="flex justify-between text-sm font-medium"><span>{r.c}</span><span className="num-pill">{r.k}</span></div>
                  <div className="h-2 rounded-full bg-secondary mt-1.5 overflow-hidden"><div className="h-full bg-teal" style={{width:r.w}}/></div>
                </div>
              ))}
            </div>
            <div className="mt-5 text-xs text-muted-foreground">Estimates based on active vendors with verified bookings · Q1 2026</div>
          </div>
        </div>
      </section>
        ...
        <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            ...
            <div key={t} className="card-soft p-6">
              <div className="size-11 rounded-xl bg-amber-soft text-foreground grid place-items-center"><Icon className="size-5"/></div>
          ...
          {[
            ...
            <div key={p.tier} className={"card-soft p-7 flex flex-col " + (p.featured ? "ring-2 ring-teal" : "")}>
              ...
              <ul className="mt-5 space-y-2 text-sm flex-1">
                {p.lines.map(l => <li key={l} className="flex gap-2"><Check className="size-4 text-teal shrink-0 mt-0.5"/>{l}</li>)}
              </ul>
              <button className={"mt-5 px-5 py-3 rounded-xl text-sm font-semibold " + (p.featured?"bg-ink text-cream":"border border-border bg-card")}>{p.cta}</button>
            </div>
          ))}
        </div>
      </section>
